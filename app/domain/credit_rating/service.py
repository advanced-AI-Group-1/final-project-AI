...
import re
import time
from .agent_validator import get_credit_rating_agent, AgentState
import asyncio
...
"""
    ...
    async def evaluate_credit_rating(self, financial_data: Dict[str, Any]) -> \
        ...
        try:
            ...
            # 1. 이미 최하위 등급인 경우
            if result["credit_rating"] in ["D", "E"]:
                needs_agent_validation = False
                logger.info(f"{company_name} 기업은 이미 최하위 등급({result['credit_rating']})으로 에이전트 검증을 건너뜁니다.")
            # 2. 완전 자본잠식 상태
            elif financial_data.get('total_equity', 1) <= 0:
                needs_agent_validation = False
                logger.info(f"{company_name} 기업은 완전 자본잠식 상태로 에이전트 검증을 건너뜁니다.")
            # 3. 부채가 자산을 초과하는 경우
            elif financial_data.get('total_liabilities', 0) > financial_data.get('total_assets', 1):
                needs_agent_validation = False
            ...
            # 5. 매우 우량한 기업인 경우
            elif result["credit_rating"] == "AAA" and \
                 financial_data.get('debt_ratio', 0) < 0.5 and \
                 financial_data.get('current_ratio', 0) > 2.0 and \
                 financial_data.get('operating_profit_margin', 0) > 0.2:
                needs_agent_validation = False
                logger.info(f"{company_name} 기업은 매우 우량한 AAA 등급으로 에이전트 검증을 건너뜁니다.")
            
            # 에이전트 검증 수행 (필요한 경우에만)
            if needs_agent_validation:
                agent_result = await self._validate_with_agent(
                    financial_data=financial_data,
                    current_rating=result["credit_rating"],
                    validation_issues=validation_issues
                )
                
                # 에이전트 검증 결과 반영
                if agent_result.get("is_adjusted", False):
                    result["original_rating"] = result["credit_rating"]
                    result["credit_rating"] = agent_result["final_rating"]
                    result["agent_adjustment_reason"] = agent_result["final_reason"]
                    result["is_adjusted"] = True
                    
                    # 조정 사유가 있는 경우 부정적 요인에 추가
                    if agent_result["final_reason"]:
                        if "negative_factors" not in result:
                            result["negative_factors"] = []
                        if isinstance(result["negative_factors"], list):
                            result["negative_factors"].append(agent_result["final_reason"])
                
                result["agent_validation_skipped"] = False
            else:
                result["agent_validation_skipped"] = True
                result["agent_adjustment_reason"] = "명확한 케이스로 인해 검증이 건너뛰어졌습니다."
            ...
            # 명확한 케이스 확인 (에이전트 검증 생략)
            if adjusted_rating in ['D']:  # D등급은 명확한 케이스로 판단
                needs_agent_validation = False
                ...
                logger.info(f"{company_name} 기업은 현금흐름 악화로 인해 에이전트 검증을 건너뜁니다.")
                 ...
                 financial_data.get('debt_ratio', 0) < 50 and \
                 financial_data.get('current_ratio', 0) > 2.0 and \
                 financial_data.get('operating_profit_margin', 0) > 0.2:
                needs_agent_validation = False
                logger.info(f"{company_name} 기업은 매우 우량한 AAA 등급으로 에이전트 검증을 건너뜁니다.")
