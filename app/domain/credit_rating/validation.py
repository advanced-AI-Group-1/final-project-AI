...
def check_debt_risk(data: Dict[str, Any]) -> Tuple[int, List[str]]:
    ...
    if data.get('operating_profit', 0) > 0 and data.get('interest_bearing_debt', 0) > 0:
        interest_coverage = data.get('operating_profit', 0) / (data.get('interest_bearing_debt', 1) * 0.05)  # 가정 이자율 5%
...
def check_manufacturing_specific(data: Dict[str, Any]) -> Tuple[int, List[str]]:
    ...
    if data.get('industry_name', '') in ['전자 부품 제조업', '기계 제조업', '자동차 제조업']:
        ...
        # 재고 관련 위험 (간접 추정)
        if data.get('net_income', 0) > 0 and data.get('operating_cash_flow', 0) < data.get('net_income', 1) * 0.5:
            risk_score += 1
...
def comprehensive_credit_validation(financial_data: Dict[str, Any], ml_prediction: str) -> Tuple[str, str]:
    """
    종합적인 신용등급 검증 시스템
    ...
    all_reasons.extend(profit_reasons + debt_reasons + industry_reasons + market_reasons + size_reasons)
    ...
    if final_score < original_score:
        adjustment_reason = f"ML예측({ml_prediction})에서 {adjusted_rating}로 하향조정. 사유: {', '.join(all_reasons)}"
        logger.info(f"신용등급 하향 조정: {ml_prediction} -> {adjusted_rating}")
    else:
        adjustment_reason = f"ML예측({ml_prediction}) 유지"
