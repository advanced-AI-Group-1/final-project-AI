...
class FinancialDataService:
		...
		try:
			...
			if csv_path is None:
				...
				csv_path = os.path.join(base_dir, 'data', 'csv',
				                        'dart_general_company_financial_fixed_en.csv')
	...
	async def search_similar_financial_data(self, prompt: str, top_k: int = 8) -> \
	List[Dict[str, Any]]:
		"""
		프롬프트와 유사한 기업 검색
		"""
		...
		results = []
		for company in search_results["results"]:
			financial_data = {key: value for key, value in
			                  company["metadata"].items()}
			# 기본 정보도 financial_data에 포함
			financial_data["corp_code"] = company["corp_code"]
			financial_data["corp_name"] = company["corp_name"]
			financial_data["market_type"] = company["market_type"]
			financial_data["industry_name"] = company["industry_name"]
	...
	def extract_conditions_from_prompt(self, prompt: str) -> Dict[str, float]:
		...
		for kor_key, num, unit, comp in matches:
			...
			# ✅ 이거 추가
			logger.info(
				f"[조건 해석] {kor_key} → {eng_key}, 수치: {num}{unit} → 환산값: {value} (billion KRW)")
	...
	def convert_extracted_conditions(self, extracted: Dict[str, float]) -> Dict[
		str, Dict[str, float]]:
		"""
	...
	async def filter_financial_data_from_prompt(self, prompt: str,
			industry: Optional[str] = None, top_k: int = 8):
		"""
		...
		search_results = await self.vector_store.search_similar_companies_with_filter(
				query=prompt,
				industry=industry,
