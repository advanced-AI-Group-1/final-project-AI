...
class ReportAgent:
	...
	async def _call_llm(self, prompt: str, section_name: str = "",
		...
		if company_name:
			# 응답 로깅
			prefix = f"section_{section_name}" if section_name else "general"
			log_to_memory(response, 'response', 'report_agent', company_name, '원',
			              prefix)
	...
	def _format_financial_data(self, financial_data: Dict[str, Any]) -> str:
		...
		# 현금흐름표
		result.append("\n■ 현금흐름표 데이터:")
		operating_cf_keys = ['operating_cash_flow', '영업활동현금흐름',
		...
		result.append(
				f"매출총자산회전율: {financial_data.get('asset_turnover_ratio', 0):.2f}회")
	...
	def _format_credit_rating(self, credit_rating: Dict[str, Any]) -> str:
		...
		if credit_rating and 'rating_details' in credit_rating and credit_rating[
			'rating_details']:
		...
		# 긍정적 요인이 있는 경우 처리
		if credit_rating and 'positive_factors' in credit_rating and credit_rating[
			'positive_factors']:
			result.append("\n[긍정적 요인]")
		...
		# 부정적 요인이 있는 경우 처리
		if credit_rating and 'negative_factors' in credit_rating and credit_rating[
			'negative_factors']:
			result.append("\n[부정적 요인]")
	...
	def _initialize_state(self, company_name: str,
	...
	async def _calculate_additional_ratios(self, state: Dict[str, Any]) -> Dict[
		str, Any]:
		"""추가 재무비율을 계산합니다."""
		...
		# 추가 재무비율 계산
		additional_ratios = AdditionalRatioCalculator.calculate_all_additional_ratios(
				company_data)
		...
		if company_name:
			...
			# 노드 결과를 메모리에 저장
			log_node_to_memory("calculate_ratios", log_content, "report_agent",
			                   company_name)

		return state

	async def _generate_summary_card(self, state: Dict[str, Any]) -> Dict[
		str, Any]:
		"""요약 카드를 생성합니다."""
    ...
    """.format(summary_card=summary_card)
		...
		if company_name:
			...
			# 노드 결과를 메모리에 저장
			log_node_to_memory("generate_summary", log_content, "report_agent",
			                   company_name)
	...
	async def _analyze_section(self, state: Dict[str, Any]) -> Dict[str, Any]:
		...
		if current_section["name"] == "재무 분석":
			...
			prompt = CREDIT_RATING_ANALYSIS_PROMPT.format(
					company_name=company_data.get("corp_name", ""),
					industry_name=company_data.get("industry_name", ""),
					market_type=company_data.get("market_type", ""),
					financial_data=self._format_financial_data(company_data),
		...
		if company_name:
			...
			# 노드 결과를 메모리에 저장
			log_node_to_memory(f"analyze_section_{current_section['name']}",
			                   log_content, "report_agent",
			                   company_name)
		...
		if regeneration_mode:
			...
			try:
				...
				if current_idx < len(sections_to_regenerate) - 1:
					state["current_section_index"] = sections_to_regenerate[
						current_idx + 1]
      ...
      """
			...
			review_result = await self._call_llm(prompt, section_name=section['name'],
			                                     company_name=company_data.get(
					                                     "corp_name", ""))
			self.logger.info(
					f"LLM 응답 (섹션 '{section['name']}'): {review_result[:100]}...")
			...
			score_lines = [line for line in review_result.split('\n') if
			               '종합' in line and '점수' in line]
			if score_lines:
				score_text = re.split(r'종합\s*점수:', score_lines[0], maxsplit=1)[
					1].strip()
				# 숫자만 추출
			...
			if score is None:
				...
				if score_lines:
					...
					if score_match:
						score = float(score_match.group(1))
						self.logger.info(
								f"방법 2로 점수 추출 성공: {score} (원본 텍스트: '{score_text}')")
				...
				for pattern in patterns:
					...
					for match in matches:
						if isinstance(match, tuple):
							...
							score_str = match[0] if pattern.startswith(r'(\d+)\.') else match[
								0]
							try:
			...
			if score is None:
				self.logger.warning(
						f"섹션 '{section['name']}'의 점수를 추출하지 못했습니다. 기본값 5.0 사용")
		...
		if company_name:
			...
			# 노드 결과를 메모리에 저장
			log_node_to_memory("review_report", log_content, "report_agent",
			                   company_name)
		...
		if sections_to_regenerate:
			self.logger.info(
					f"재생성이 필요한 섹션: {[sections[i]['name'] for i in sections_to_regenerate]}")
	...
	async def _compile_report(self, state: Dict[str, Any]) -> Dict[str, Any]:
		...
		if company_name:
			...
			# 노드 결과를 메모리에 저장
			log_node_to_memory("compile_report", log_content, "report_agent",
			                   company_name)

			# 최종 보고서 내용을 별도로 노드 로그에 추가
			log_node_to_memory("final_report", detailed_report, "report_agent",
			                   company_name)
	...
	async def _plan_sections(self, state: Dict[str, Any]) -> Dict[str, Any]:
		...
		if company_name:
			...
			# 노드 결과를 메모리에 저장
			log_node_to_memory("plan_sections", log_content, "report_agent",
			                   company_name)
	...
	async def generate_report(self, company_name: str,
			credit_rating_result: Dict[str, Any],
		...
		# 초기 상태 생성
		initial_state = self._initialize_state(company_name, credit_rating_result,
		                                       financial_data)
		...
		try:
			...
			if hasattr(initial_state, "model_dump"):
				...
				# 둘 다 없는 경우 직접 변환
				state_dict = {k: v for k, v in initial_state.__dict__.items() if
				              not k.startswith('_')}
			...
			end_time = time.time()
			self.logger.info(
					f"'{company_name}' 기업 보고서 생성 완료 (소요 시간: {end_time - start_time:.2f}초)")
...
class SSEReportAgent(ReportAgent):
	"""
	SSE 이벤트를 지원하는 보고서 생성 에이전트
	ReportAgent를 상속받아 SSE 이벤트를 발생시키는 기능을 추가합니다.
	"""
	...
	def set_progress_callback(self, callback):
		"""
		진행 상황을 보고할 콜백 함수를 설정합니다.

		Args:
				callback: 진행 상황 콜백 함수 (message, progress)
		"""
		self.progress_callback = callback

	async def _calculate_additional_ratios(self, state: Dict[str, Any]) -> Dict[
		str, Any]:
		"""추가 재무비율을 계산하고 진행 상황을 보고합니다."""
		if self.progress_callback:
			self.progress_callback("추가 재무비율 계산 중...", 30)

		return await super()._calculate_additional_ratios(state)

	async def _generate_summary_card(self, state: Dict[str, Any]) -> Dict[
		str, Any]:
		"""요약 카드를 생성하고 진행 상황을 보고합니다."""
		if self.progress_callback:
			self.progress_callback("보고서 요약 카드 생성 중...", 40)

		return await super()._generate_summary_card(state)

	async def _plan_sections(self, state: Dict[str, Any]) -> Dict[str, Any]:
		"""보고서 섹션을 계획하고 진행 상황을 보고합니다."""
		if self.progress_callback:
			self.progress_callback("보고서 섹션 계획 중...", 20)

		return await super()._plan_sections(state)

	async def _analyze_section(self, state: Dict[str, Any]) -> Dict[str, Any]:
		"""현재 섹션을 분석하고 진행 상황을 보고합니다."""
		sections = state["sections"]
		current_index = state["current_section_index"]
		...
		if current_index < len(sections):
			current_section = sections[current_index]
			if self.progress_callback:
				# 섹션 분석 진행률 계산 (50%~80% 사이에서 분배)
				total_sections = len(sections)
				progress = 50 + (current_index / total_sections) * 30
				self.progress_callback(f"보고서 '{current_section['name']}' 섹션 작성 중...",
				                       int(progress))

		return await super()._analyze_section(state)

	async def _review_report(self, state: Dict[str, Any]) -> Dict[str, Any]:
		"""보고서 품질을 검증하고 진행 상황을 보고합니다."""
		if self.progress_callback:
			self.progress_callback("보고서 품질 검증 중...", 85)

		return await super()._review_report(state)

	async def _compile_report(self, state: Dict[str, Any]) -> Dict[str, Any]:
		"""최종 보고서를 컴파일하고 진행 상황을 보고합니다."""
		if self.progress_callback:
			self.progress_callback("최종 보고서 컴파일 중...", 95)

		result = await super()._compile_report(state)

		if self.progress_callback:
			self.progress_callback("보고서 생성 완료", 100)
	...
	async def generate_report(self, company_name: str,
		...
		"""
		if self.progress_callback:
			self.progress_callback(f"{company_name} 기업의 보고서 생성을 시작합니다.", 10)

		return await super().generate_report(company_name, credit_rating_result,
		                                     financial_data)
